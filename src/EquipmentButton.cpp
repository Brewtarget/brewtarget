/*
* EquipmentButton.cpp is part of Brewtarget, and is Copyright Philip G. Lee
...
void EquipmentButton::setEquipment(Equipment* equip)
{
   ...
   if( _equip )
   {
      connect( _equip, SIGNAL(changed(QMetaProperty,QVariant)), this, SLOT(recChanged(QMetaProperty,QVariant)) );
      setText( _equip->name() );
   }
   else
      setText("");
