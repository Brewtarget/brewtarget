...
QMap<QString, Unit*> FahrenheitTempUnitSystem::nameToUnit;
...
double FahrenheitTempUnitSystem::qstringToSI( QString qstr )
{
   ...
   if( listSize < 1 ) // Didn't even provide a number.
      return 0.0;
   else if( listSize < 2  ) // Only provided a number. Assume it's in F.
      return Units::fahrenheit->toSI(amt);
   else // Provided a number and unit.
   {
      Unit* u = getUnit(list1[1]);

      if( u == 0 ) // Invalid unit since it's not in the map.
         return Units::fahrenheit->toSI(amt); // Assume units are F.
      else
         return u->toSI(amt);
   }
}
...
Unit* FahrenheitTempUnitSystem::getUnit(const QString& name)
{
